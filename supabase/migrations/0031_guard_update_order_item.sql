-- Migration: 0031_guard_update_order_item.sql
-- Description: Prevent inline item edits on orders already in kitchen/prepared; direct users to alter_order_v2.
-- Dependencies: 0016_add_item_operations.sql
...
BEGIN
    ...
    -- Guard: once in kitchen/prepared, force alter_order_v2 instead of inline edits
    IF v_order_status IN ('InKitchen', 'Prepared') THEN
        RAISE EXCEPTION 'Order is already being prepared. Use alter_order_v2 instead.';
    END IF;
