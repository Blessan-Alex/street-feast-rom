-- Migration: 0027_fix_orders_upsert_table_check.sql
-- Description: Fix orders_upsert to skip table availability check when updating existing order
-- Dependencies: 0012

BEGIN;

-- Update orders_upsert to only check table availability for NEW orders, not updates
-- When updating an existing order, we don't need to check availability
-- because we're modifying the same order that already occupies the table
...
begin
    ...
    -- Only validate table availability for NEW DineIn orders (not updates)
    -- When updating an existing order, we don't need to check availability
    -- because we're modifying the same order that already occupies the table
    if v_type = 'DineIn' and v_table_number is not null and v_existing_order_id is null then
