    ...
    init {
        ...
        viewModelScope.launch {
            try {
                ...
                val result = repository.maybeRefreshMenu(actualStoreId)
                result.onSuccess { menuData ->
                    Log.d("MenuViewModel", "Menu refresh success: ${menuData?.categories?.size ?: 0} categories, ${menuData?.items?.size ?: 0} items")
                }
                result.onFailure { e ->
                ...
                repository.subscribeToMenuUpdates(viewModelScope, actualStoreId) { menuData ->
                    Log.d("MenuViewModel", "Menu updated via realtime: ${menuData.categories.size} categories, ${menuData.items.size} items")
