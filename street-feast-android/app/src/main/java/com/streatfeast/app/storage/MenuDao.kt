...
interface MenuDao {
    ...
    // Categories
    @Query("SELECT * FROM categories WHERE storeId = :storeId AND isActive = 1 ORDER BY name")
    ...
    // Items
    @Query("SELECT * FROM items WHERE storeId = :storeId AND isActive = 1 ORDER BY name")
    fun getAllItems(storeId: String): Flow<List<ItemEntity>>
    
    @Query("SELECT * FROM items WHERE storeId = :storeId AND isActive = 1 ORDER BY name")
    suspend fun getAllItemsSync(storeId: String): List<ItemEntity>
    
    @Query("SELECT * FROM items WHERE categoryId = :categoryId AND isActive = 1 ORDER BY name")
